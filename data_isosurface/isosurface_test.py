...
def heuristic_density(n, l, m):
    ...
    if l == 0:
        ...
        if l == n - 1 and abs(m) == l: d *= 0.9
        if l == n - 2 and abs(m) == l: d *= 0.8
    return d / n**6
...
'3 sigma u *': 0.0051
"""
iso_val = np.sqrt(heuristic_density(n, mu, l)) / 10.0
...
norm = ((vol**2).sum()) * (del_xy**2) * del_z
print(f'Should equal 1: {norm}')

# iso_val = 0.002
# iso_val = np.sqrt(heuristic_density(n, mu, l)) / 10.0 # fix this later
...
def percentage(iso_val):

    pos_values = vol[vol > iso_val]
    neg_values = vol[vol < -iso_val]
    
    result = (pos_values**2).sum() + (neg_values**2).sum()
    result *= (del_xy**2) * del_z
    
    print(f'Percentage: {(result/norm)*100:.2f}%')
...
ax.plot_trisurf(verts1[:, 0] - xy_range, 
                verts1[:, 1] - xy_range, 
                faces1, 
                verts1[:, 2] - z_range,
...
if n + mu + l > 1: 
    ...
    ax.plot_trisurf(verts2[:, 0] - xy_range, 
                    verts2[:, 1] - xy_range, 
                    faces2, 
                    verts2[:, 2] - z_range,
                    color='blue', lw=1, alpha=0.6)

ax.set_box_aspect((del_xy, del_xy, del_z))
plt.show()
